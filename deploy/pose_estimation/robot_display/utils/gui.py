    ...
    def create_widgets(self):
        for i, name in enumerate(self.labels):
            ...
            tk.Label(
                ...
                width=max([len(name) for name in self.labels]) + 5,
            ).pack(side=tk.LEFT)
            ...
            entry = tk.Entry(
                frame, width=8, validate="key", font=self.entry_font, validatecommand=vcmd
            )
        ...
        if self.save_callback is not None:
            tk.Button(
                button_frame, text="Save", font=("Arial", 12), command=self.save, width=10
            ).pack(side=tk.RIGHT, pady=10)

        if self.reset_callback is not None:
            tk.Button(
                button_frame, text="Reset", font=("Arial", 12), command=self.reset, width=10
            ).pack(side=tk.RIGHT, pady=10)
...
def start_gui(
    cfg: dict, values: list[float], save_callback: Callable = None, reset_callback: Callable = None
) -> threading.Event:
    """
...
if __name__ == "__main__":
    ...
    start_gui(
        cfg={
            "label": ["joint1", "joint2", "joint3"],
            "range": {"joint1": [-1, 1], "joint2": [-1, 1], "joint3": [-1, 1]},
        },
        values=values,
