...
class Display(Robot):
    def __init__(self, cfg, vis_options=None):
        ...
        super().__init__(
            ...
            foot_names=[
                self.cfg.robot_args.left_foot_link_name,
                self.cfg.robot_args.right_foot_link_name,
            ],
        ...
        if "dof_names" in vars(self.cfg.robot_args).keys():
            assert len(self.cfg.robot_args.dof_names) == self.num_dofs, (
                "Number of dof names should match the number of dofs"
            )
