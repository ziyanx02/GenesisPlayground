...
class Display(Robot):
    def __init__(self, cfg: dict, vis_options=None):
        ...
        if "dof_names" in self.cfg["environment"].keys():
            assert len(self.cfg["environment"]["dof_names"]) == self.num_dofs, "Number of dof names should match the number of dofs"
            self.set_dof_order(self.cfg["environment"]["dof_names"])
