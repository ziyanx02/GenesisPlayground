...
def main(args: argparse.Namespace) -> None:
    ...
    real_env = CustomEnv(args=env_args, num_envs=1, show_viewer=False, device=device)
    link_idx_local = real_env.get_link_idx_local_by_name("imu_in_torso")
    real_env.set_link_pose(link_idx_local, quat=torch.tensor([1.0, 0.0, 0.0, 0.0]))
    ...
    for i in range(period * NUM_TOTAL_PERIODS):
        ...
        action += torch.randn(29, dtype=torch.float32) * 0.03

        link_idx_local = real_env.get_link_idx_local_by_name(args.link)
        link_pos = real_env.get_link_pose(link_idx_local)[0].cpu().numpy()
