...
class MotionLib:
        ...
        for i in tqdm(range(num_motion_files), desc="[MotionLib] Loading motions"):
            ...
            try:
                with open(curr_file, "rb") as f:
                    ...
                    root_lin_vel = torch.zeros_like(root_pos)  # (num_frames, 3)
                    root_lin_vel[:-1, :] = fps * (root_pos[1:, :] - root_pos[:-1, :])
                    root_lin_vel[-1, :] = root_lin_vel[-2, :]
                    root_lin_vel = smooth(root_lin_vel, 19, device=self._device)
                    ...
                    motion_root_pos.append(root_pos)
                    motion_root_pos_delta.append(root_pos_delta)
                    motion_root_rot.append(root_rot)
                    motion_root_lin_vel.append(root_lin_vel)
                    motion_root_ang_vel.append(root_ang_vel)
                    motion_dof_pos.append(dof_pos)
                    motion_dof_vel.append(dof_vel)

                    motion_local_body_pos.append(local_body_pos)
            except Exception as e:
                logger.error("Error loading motion file %s: %s", curr_file, e)
                continue
    ...
    def sample_motions(self, n: int, motion_difficulty: torch.Tensor | None = None) -> torch.Tensor:
    ...
    def calc_motion_frame(
        ...
        root_lin_vel = self._motion_root_lin_vel[frame_idx0]
        ...
        return root_pos, root_rot, root_lin_vel, root_ang_vel, dof_pos, dof_vel, local_key_body_pos
