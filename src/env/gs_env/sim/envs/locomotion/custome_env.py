...
class CustomEnv(BaseEnv):
    ...
    def reset_idx(self, envs_idx: torch.IntTensor) -> None:
        ...
        random_euler[:, :2] = (torch.rand(len(envs_idx), 2, device=self._device) - 0.5) * 0.0
        random_euler[:, 2] = -np.pi / 2
        quat = quat_from_euler(random_euler)
        quat = quat_mul(quat, default_quat)
        dof_pos = (
            default_dof_pos
            + (torch.rand(len(envs_idx), self._robot.dof_dim, device=self._device) - 0.5) * 0.0
        )
    ...
    def apply_action(self, action: torch.Tensor) -> None:
        action = action.detach().to(self._device) * 0
        self._action = action
