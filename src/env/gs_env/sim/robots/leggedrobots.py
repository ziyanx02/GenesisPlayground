...
class LeggedRobotBase(BaseGymRobot):
    ...
    def _randomize_rigids(self, envs_idx: torch.IntTensor):
        # friction
        min_friction, max_friction = self._args.dr_args.friction_range
        solver: RigidSolver = self._robot.solver
        ratios = torch.rand(len(envs_idx), 1).repeat(1, solver.n_geoms) \
        ...
        added_mass = torch.rand(len(envs_idx), 1) \
                     * (max_mass - min_mass) + min_mass
        solver.set_links_mass_shift(added_mass, [self._body_link_idx,], envs_idx)
        # com displacement
    ...
    def _randomize_controls(self, envs_idx: torch.IntTensor):
        ...
        self._motor_offset[envs_idx] = torch.rand(len(envs_idx), self._dof_dim)\
    ...
    def _apply_dr_joint_pos(self, act: DRJointPosAction) -> None:
        ...
        q_force = (
            self._batched_dof_kp * (act.joint_pos + self._default_dof_pos - self._dof_pos + self._motor_offset)
