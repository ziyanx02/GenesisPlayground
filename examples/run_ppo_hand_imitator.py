#!/usr/bin/env python3
"""Example: Train PPO on WUJI Hand Trajectory Imitation task using Genesis RL.

This script trains a policy to follow reference hand trajectories from demonstrations,
following the ManipTrans DexHandImitator architecture.
"""
...
def evaluate_policy(
    ...
    def evaluate() -> None:
        ...
        # For tracking hand trajectory following
        wrist_pos_errors = []
        wrist_rot_errors = []
        trajectory_progress = []
        ...
        while True:
            ...
            if step_count % 50 == 0:
                ...
                if wrist_pos_errors:
                    avg_pos_error = np.mean(wrist_pos_errors[-50:])
                    avg_rot_error = np.mean(wrist_rot_errors[-50:])
                    print(f"  Avg wrist pos error (last 50 steps): {avg_pos_error:.4f} m")
                    print(f"  Avg wrist rot error (last 50 steps): {avg_rot_error:.4f}")
        ...
        if wrist_pos_errors:
            avg_pos_error = np.mean(wrist_pos_errors)
            avg_rot_error = np.mean(wrist_rot_errors)
            print(f"Average wrist position error: {avg_pos_error:.4f} m")
            print(f"Average wrist rotation error: {avg_rot_error:.4f}")
        if trajectory_progress:
...
def main(
    ...
    num_ckpt: int | None = None,
    use_wandb: bool = True,
    env_name: str = "wuji_hand_imitator",
    ...
    **cfg_overrides: Any,
    ...
    --trajectory_path=path/to/your/trajectory.pkl
    --algo.lr=1e-3
