#!/usr/bin/env python3
"""Example: Train PPO on WUJI Hand In-Hand Rotation task using Genesis RL."""
...
import numpy as np
import torch
from gs_agent.algos.config.registry import PPO_INHAND_ROTATION_MLP
from gs_agent.algos.ppo import PPO
from gs_agent.runners.config.registry import RUNNER_INHAND_ROTATION_MLP
...
def evaluate_policy(
    ...
    def evaluate() -> None:
        ...
        # For tracking cube rotation
        cube_rotations = []
        cube_z_angular_velocities = []
        ...
        while True:
            ...
            # Track cube rotation
            if hasattr(wrapped_env.env, "cube_euler"):
                cube_rotations.append(wrapped_env.env.cube_euler[0, 2].cpu().item())
            if hasattr(wrapped_env.env, "cube_ang_vel"):
                cube_z_angular_velocities.append(wrapped_env.env.cube_ang_vel[0, 2].cpu().item())
            ...
            if step_count % 50 == 0:
                ...
                if cube_z_angular_velocities:
                    avg_ang_vel = np.mean(cube_z_angular_velocities[-50:])
                    print(f"  Avg Z angular velocity (last 50 steps): {avg_ang_vel:.3f} rad/s")
        ...
        # Plot cube rotation if data available
        if cube_rotations and not show_viewer:
            print("Plotting cube rotation statistics...")
            fig, axes = plt.subplots(2, 1, figsize=(12, 8))

            steps = np.arange(len(cube_rotations))

            # Cube Z rotation over time
            plot_metric_on_axis(
                axes[0],
                steps,
                [cube_rotations],
                ["Z Rotation"],
                "Rotation (rad)",
                "Cube Z-Axis Rotation Over Time",
            )

            # Cube Z angular velocity over time
            if cube_z_angular_velocities:
                plot_metric_on_axis(
                    axes[1],
                    steps,
            ...
            plot_path = plot_dir / f"{num_ckpt}_rotation.png"
            plt.tight_layout()
            plt.savefig(plot_path, dpi=150)
            plt.close(fig)
            print(f"Rotation plot saved to: {plot_path}")
        ...
        if cube_rotations:
            total_rotation = cube_rotations[-1] - cube_rotations[0]
            print(f"Total Z rotation: {total_rotation:.2f} rad ({np.degrees(total_rotation):.1f} deg)")
...
def main(
    ...
    **cfg_overrides: Any,
    ...
    --reward_args.CubeZRotationVelocityReward=15
    ...
    algo_cfg = apply_overrides_generic(PPO_INHAND_ROTATION_MLP, algo_overrides, prefixes=("cfgs.", "algo."))
    runner_args = apply_overrides_generic(
        RUNNER_INHAND_ROTATION_MLP, runner_overrides, prefixes=("cfgs.", "runner.")
    )
