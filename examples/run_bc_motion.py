...
def create_bc_runner_from_registry(
    ...
    if teacher_ckpt is not None:
        teacher_ckpt_path = Path(teacher_exp_dir) / "checkpoints" / f"checkpoint_{teacher_ckpt:04d}.pt"
    ...
    algo_cfg = algo_cfg.model_copy(update={
...
def train_policy(
    ...
    print(f"Note: BC uses teacher observations (from teacher config) for teacher policy.")
    print(f"Student uses student observations. Teacher provides action labels.")

    # Train using Runner
    print("Starting BC training...")
    print(f"Student environment: g1_motion")
