...
from gs_agent.algos.config.registry import PPO_TELEOP_TEACHER_MLP
from gs_agent.algos.ppo import PPO
from gs_agent.runners.config.registry import RUNNER_TELEOP_TEACHER_MLP
...
from gs_env.sim.envs.locomotion.teleop_teacher_env import TeleopTeacherEnv


def create_gs_env(
    env_name: str = "teleop_teacher_default",
...
def create_ppo_runner_from_registry(env: BaseEnv, exp_name: str | None = None) -> OnPolicyRunner:
    ...
    ppo = PPO(
        env=wrapped_env,
        cfg=PPO_TELEOP_TEACHER_MLP,
        device=wrapped_env.device,
    )

    runner_args = RUNNER_TELEOP_TEACHER_MLP
    if exp_name is not None:
        # Avoid mutating a frozen Pydantic model; create a copied config with updated save_path
        runner_args = RUNNER_TELEOP_TEACHER_MLP.model_copy(
...
def evaluate_policy(
    ...
    if exp_name is not None:
        ...
        log_patterns = ["logs/ppo_gs_teleop/*"]
        log_dirs = []
        for pattern in log_patterns:
            ...
            if log_dirs:
                break
        if not log_dirs:
            raise FileNotFoundError(f"No experiment directories found. Tried patterns: {log_patterns}")
    ...
    if num_ckpt is not None:
        ...
        ckpt_path = load_latest_model(exp_dir)
...
def train_policy(
    ...
    if exp_name is not None:
        ...
        save_path = RUNNER_TELEOP_TEACHER_MLP.save_path
...
def main(num_envs: int = 2048,
    show_viewer: bool = False,
    device: str = "cuda",
    eval: bool = False,
    exp_name: str | None = None,
    num_ckpt: int | None = None,) -> None:
